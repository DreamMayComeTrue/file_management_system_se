...
// Audit role: export completion report as CSV
exports.exportReport = asyncHandler(async (req, res) => {
  const [rows] = await pool.query(
    `SELECT
       sub.code, sub.name AS subjectName,
       sec.name AS sectionName, sec.deadline,
  ...
  const csvBody = rows.map(r =>
    [r.code, r.subjectName, r.sectionName, r.deadline ?? '', r.totalSubfolders, r.completed, r.status].join(',')
  ).join('\n')
